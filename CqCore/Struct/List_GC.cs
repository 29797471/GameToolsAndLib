...
namespace CqCore
{
    ...
    ///  3. T[] 支持泛型 ,不可动态扩容
    /// </summary>
    public class List_GC<T> : IEnumerable<T>
    {
        ...
        public List_GC(int capacity, int enumCount = 5)
        {
            init(enumCount);
        }
        ...
        public IEnumerator<T> GetEnumerator()
        {
            ...
            if (idleEnumID < 0)
            {
                throw new Exception("use too much enumerators");
            }
            //标记为已经使用状态
        ...
        public void Sort(ComparisonFloatPriority<T> cp)
        {
            Sort(new FunctorComparer<T>(cp));
        }
        public void Sort(ComparisonPriority<T> cp)
        {
            Sort(new FunctorComparer<T>(cp));
        }

        void Sort(FunctorComparer<T> funComparer)
        {
            Array.Sort(m_items, 0, m_length, funComparer);
        }

        struct ABEnumerator<T1> : IDisposable, IEnumerator<T>
        {
            private List_GC<T> m_list;
            ...
            internal ABEnumerator(List_GC<T> list, int id)
            {
                this.m_list = list;
                this.m_idNext = 0;
                this.m_id = id;
