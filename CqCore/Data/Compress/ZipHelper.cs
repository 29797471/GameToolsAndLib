...
namespace CqCore
{
    ...
    public static class ZipHelper
    {
        ...
        /// 压缩文件夹
        /// </summary>
        public static void ZipDir(string folderPath, string zipFile, int compressionLevel = 9)
        {
            if (folderPath[folderPath.Length - 1] != Path.DirectorySeparatorChar)
            {
                folderPath += Path.DirectorySeparatorChar;
            }
            ZipOutputStream outstream = new ZipOutputStream(File.Create(zipFile));
            outstream.SetLevel(compressionLevel);
            ZipCompress(folderPath, outstream, folderPath);
            outstream.Finish();
            outstream.Close();
        }
       
        static void ZipCompress(string strFile, ZipOutputStream outstream, string staticFile)
        {
            ...
            foreach (string file in filenames)
            {
                ...
                else
                {
                    ...
                    //此步极为关键,主要处理安卓移动平台解压时路径错误.
                    tempfile= tempfile.Replace(@"\","/");
                    //CqDebug.Log("tempFile:" + tempfile);
