...
/// 5.自定义类
/// </summary>
internal class TorsionParser : TokenParser
{
    public TorsionParser(List<Token> list) : base(list)
    {
    }
    ...
    bool TryParseObject(Type type, out object v)
    {
        ...
        while (Value.type != (TokenType.RIGHT_BRACE))
        {
            ...
            if (mi!=null)
            {
                if (!TryParseLogic(TryParseValue, mi.GetMemberType(), out o))
                {
                    return false;
                }
                else
            ...
            {
                int layer = 0;
                while(layer!=0 || Value.type != TokenType.SEMICOLON)
                {
                    switch(Value.type)
                    {
